...
- **repository-structure.md** - リポジトリ構造定義署
  - フォルダ・ファイル構成
  - ディレクトリの役割
  - ファイル配置ルール
...
```bash
mkdir -p .steering/[YYYMMDD]-initial-implementation
```
...
**機能追加・修正・バグ修正を行う場合は、必ずステアリングドキュメントを作成すること。**

実装を開始する前に、以下の手順を必ず実行してください：

1. `.steering/[YYYYMMDD]-[開発タイトル]/` ディレクトリを作成
2. `requirements.md` - 要求内容を記述
3. `design.md` - 設計を記述
4. `tasklist.md` - タスクリストを作成

これはどんな小さな変更でも例外なく適用されます。ステアリングドキュメントなしに実装を開始してはいけません。